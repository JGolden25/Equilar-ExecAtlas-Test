...
for ($i = 0; $i < 10; $i++) {
    $eventTypes = ["NEW_POSITION", "TITLE_CHANGE", "RESIGNATION", "REMOVED_FROM_WEBSITE"];
    $sources = ["TAGGER", "PRESS_RELEASE", "WEBSITE_CHECKER", "PE_INFO", "COMPANIES_HOUSE", "CRUNCHBASE"];
    
    $mockData[] = [
        "personName" => "Executive " . ($i + 11),
        "title" => ["CEO", "CFO", "CTO", "COO", "CMO", "CHRO", "CIO", "CLO", "President", "EVP"][rand(0, 9)],
        "org" => ["name" => "Company " . chr(65 + $i) . " Inc."],
        "eventType" => $eventTypes[rand(0, 3)],
        "source" => $sources[rand(0, 5)],
        "effectiveDate" => date('Y-m-d', strtotime('-' . rand(1, 60) . ' days')),
        "reportedDate" => date('Y-m-d', strtotime('-' . rand(1, 30) . ' days')),
        "profileImage" => null
    ];
}
